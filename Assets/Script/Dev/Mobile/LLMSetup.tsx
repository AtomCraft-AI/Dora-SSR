...
import { App, DB, Director, HttpServer, Label, Node, TextAlign, Move, Opacity, Spawn, Ease, Vec2, thread, sleep } from "Dora";
...
import { SceneSurface as RoundedSurface } from "Dev/Mobile/Visual";
...
export const mobileLLMPresets: MobileLLMPreset[] = [
	...
	},
	{
		...
		maxTokens: 8192,
		customOptions:
			'{"max_tokens":null,"max_completion_tokens":8192,"top_p":0.95,"auxiliaryOptions":{"max_tokens":null,"max_completion_tokens":8192,"reasoning_effort":null,"thinking":{"type":"disabled"}}}',
	},
	{
		id: "zai",
		name: "ZAI",
...
export const isMobileLLMOpen = () => activeSetup !== undefined && activeSetup.parent !== undefined;
...
export function startMobileLLMSetup(options: MobileLLMSetupOptions) {
	...
	// The full-screen modal intercepts input while the underlying page stays visible.
	...
	const save = () => {
		...
		if (key === "") {
			error = zh ? "请粘贴 API Key" : "Paste an API key";
			render();
			return;
		}
		ensureLLMConfigTable();
		...
		const affected = DB.exec(
			...
			[name, preset.url, preset.model, key, preset.contextWindow, 0.1, preset.maxTokens, "", preset.customOptions, 1, 1, now, now],
		);
		if (affected < 0) {
			error = zh ? "配置保存失败，请重试" : "Could not save the configuration";
			render();
			return;
		}
		...
		if (!id) {
			error = zh ? "无法读取新配置" : "Could not read the new configuration";
			render();
			return;
		}
	...
	const render = () => {
		...
		const preset = mobileLLMPresets[presetIndex];
		const scene = toNode(
			<node order={10000} x={-width / 2} y={-height / 2} width={width} height={height} anchorX={0} anchorY={0}>
				<node
					...
					onTapped={close}
				>
					<RoundedSurface width={width} height={height} radius={0} fillColor={0x382b3026} renderOrder={0} />
				</node>
				<node
				...
				>
					...
					<label
						...
						fontSize={18}
						text={zh ? "从模板新增配置" : "Add from template"}
						color3={0x30352b}
					/>
					<label
						x={sheetWidth - 20}
						...
						fontName={fontName}
						fontSize={12}
						text={compactText(preset.model, shortLandscape ? 42 : 24)}
						color3={0x7c826f}
					/>
					<label
						x={20}
					...
					<MobileButton
						...
						onTapped={() => choose(1)}
					/>
					<label x={keyX} y={keyLabelY} anchorX={0} anchorY={1} fontName={fontName} fontSize={14} text="API Key" color3={0x7c826f} />
					<node
						tag="mobile-llm-key"
						ref={keyRef}
						renderOrder={10}
	...
	attachGamepad(host, {
		...
		onBack: () => {
			if (keyInput.isFocused()) keyInput.blur();
			else close();
		},
	});
	...
	render();
	if (!App.reducedMotion) host.perform(Spawn(Opacity(0.18, 0, 1), Move(0.26, Vec2(0, -24), Vec2.zero, Ease.OutCubic)));
	return host;
}
...
export function getMobileLLMSelection() {
	const configs = getMobileLLMConfigs();
	const rows = DB.query("select value_num from Config where name = 'mobileRemixLLMConfigId' limit 1") as unknown[][] | undefined;
	const remembered = rows && rows.length > 0 ? tonumber(rows[0][0]) : undefined;
	return configs.find((item) => item.id === remembered)?.id ?? configs[0]?.id ?? 0;
}

function fitConfigText(value: string, size: number, width: number) {
	const measure = Label(fontName, size, true)!;
	measure.text = value;
	...
	let low = 0,
		high = inputLength(value);
	while (low < high) {
		const mid = math.floor((low + high + 1) / 2);
		measure.text = inputSlice(value, 0, mid) + "…";
		if (measure.width <= width) low = mid;
		else high = mid - 1;
	}
	measure.cleanup();
...
export function startMobileLLMManager(options: MobileLLMManagerOptions) {
	...
	const detailKeyInput = createTextInput({
		...
		getPlaceholder: () => (zh ? "粘贴新的 API Key" : "Paste a new API key"),
		isEnabled: canEdit,
		onReturn: () => true,
	});
	const close = (notify = true) => {
		...
		if (!notify || App.reducedMotion) finish();
		else {
			host.perform(Opacity(0.16, host.opacity, 0));
			thread(() => {
				sleep(0.16);
	...
	const saveDetailKey = () => {
		...
		if (key === "") {
			detailError = zh ? "请粘贴新的 API Key" : "Paste a new API key";
			render();
			return;
		}
		const affected = DB.exec("update LLMConfig set api_key = ?, updated_at = ? where id = ?", [key, os.time(), detailId]);
		if (affected <= 0) {
			detailError = zh ? "API Key 保存失败" : "Could not save the API key";
			render();
			return;
		}
		detailKeyInput.blur();
	...
	const deleteDetail = () => {
		...
		if (affected <= 0) {
			detailError = zh ? "删除失败，请重试" : "Could not delete the configuration";
			render();
			return;
		}
		...
		if (selectedId === deletingId) {
			...
			if (selectedId > 0)
				DB.exec("insert or replace into Config(name, value_num, value_str, value_bool) values('mobileRemixLLMConfigId', ?, NULL, NULL)", [
					selectedId,
				]);
	...
	const render = () => {
		...
		const safe = App.safeArea;
		const shortLandscape = safe.width >= 760 && safe.height < 500;
		const listHeight = math.min(configs.length * 68, math.max(68, math.min(420, safe.height * 0.62 - 110)));
		const desiredHeight = detailId > 0 ? 410 : 154 + listHeight;
		const sheetWidth = shortLandscape ? math.min(720, safe.width - 24) : safe.width;
		...
		const scene = toNode(
			<node order={10000} x={-width / 2} y={-height / 2} width={width} height={height} anchorX={0} anchorY={0}>
				...
				>
					...
					{detail ? (
						detailMode === "key" ? (
							<node tag="mobile-llm-detail-key">
								<label
									x={20}
									y={sheetHeight - 24}
									anchorX={0}
									...
									fontName={goTheme.headingFont}
									fontSize={18}
									text={zh ? "修改 API Key" : "Update API key"}
									color3={0x30352b}
								/>
								<label
									x={20}
									...
									fontName={fontName}
									fontSize={13}
									text={compactText(detail.name, shortLandscape ? 44 : 28)}
									color3={0x7c826f}
								/>
								<label
									x={20}
									...
									fontName={fontName}
									fontSize={13}
									text={zh ? "原 Key 不会显示，保存后立即替换" : "The current key stays hidden and will be replaced"}
									color3={0x7c826f}
								/>
								<node
							...
							<node tag="mobile-llm-detail-delete">
								<label
									x={20}
									y={sheetHeight - 24}
									anchorX={0}
									...
									fontName={goTheme.headingFont}
									fontSize={18}
									text={zh ? "删除这个配置？" : "Delete this configuration?"}
									color3={0x30352b}
								/>
								<label
									x={20}
									...
									fontName={fontName}
									fontSize={16}
									text={compactText(detail.name, shortLandscape ? 52 : 32)}
									color3={0xffff8585}
								/>
								<label
									x={20}
									...
									text={
										zh
											? "删除后无法恢复；若它是当前配置，将自动切换到下一项。"
											: "This cannot be undone. The next configuration will become active."
									}
									textWidth={contentWidth}
									alignment={TextAlign.Left}
									color3={0x7c826f}
							...
							<node tag="mobile-llm-detail">
								...
								<label
									...
									fontName={fontName}
									fontSize={15}
									text={compactText(detail.model, shortLandscape ? 56 : 38)}
									color3={0x30352b}
								/>
								<label
									x={20}
								...
								<label
									...
									fontName={fontName}
									fontSize={13}
									text={compactText(detail.url, shortLandscape ? 72 : 42)}
									color3={0x30352b}
								/>
								<label
									x={20}
									...
									fontName={fontName}
									fontSize={13}
									text={`${zh ? "上下文" : "Context"}  ${detail.contextWindow}   ·   Function Call  ${detail.supportsFunctionCalling ? (zh ? "支持" : "On") : zh ? "不支持" : "Off"}`}
									color3={0x7c826f}
								/>
								<MobileButton
								...
								<MobileButton
									...
									renderOrder={10}
									onTapped={() => select(detail.id)}
								/>
							</node>
						)
					) : (
						<node tag="mobile-llm-list">
							<label
								x={20}
								y={sheetHeight - 24}
								anchorX={0}
							...
							<label
								...
								text={
									options.taskRunning
										...
										: zh
											? "选择模型，或管理 API Key"
											: "Choose a model or manage API keys"
								}
								color3={0x7c826f}
							/>
							<node
							...
							<custom-node
								onCreate={() => {
									...
									const rows = configs.map((item, index) => {
										const y = listHeight - 60 - index * 68;
										const selected = item.id === selectedId;
										return toNode(
											<node
												key={`${item.id}`}
												tag={`mobile-llm-config-${item.id}`}
												x={0}
												y={y}
												width={contentWidth}
												height={54}
												...
												touchEnabled={true}
												swallowTouches={true}
												onTapped={() => select(item.id)}
											>
												<RoundedSurface
													width={contentWidth}
													height={54}
													radius={10}
													fillColor={selected ? 0xfff7f2d9 : 0x00000000}
													borderWidth={selected ? 1 : 0}
													borderColor={0xffd4c794}
												/>
												<GoIcon name={selected ? "checked" : "circle"} x={11} y={18} size={18} color={selected ? 0xff9c7a1c : 0xff9a9e8c} />
												<label
													x={38}
													y={34}
													anchorX={0}
													fontName={fontName}
													fontSize={14}
													text={fitConfigText(item.name, 14, contentWidth - (selected ? 128 : 88))}
													color3={0x30352b}
												/>
												<label
													x={38}
												...
												<node
													...
													touchEnabled={true}
													swallowTouches={true}
													onTapped={() => openDetail(item.id)}
												>
													<GoIcon name="next" x={17} y={18} size={18} />
												</node>
											</node>,
										)!;
									});
									for (const row of rows) scroll.view.addChild(row);
									return scroll;
								}}
							/>
							<MobileButton
								...
								width={contentWidth}
								icon="plus"
								text={zh ? "添加配置" : "Add configuration"}
								renderOrder={10}
								onTapped={add}
							/>
						</node>
