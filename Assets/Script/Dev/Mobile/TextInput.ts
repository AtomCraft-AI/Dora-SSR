...
import { roundedRectVerts } from "Dev/Mobile/Visual";
...
// Advances come from the rendered font so proportional text and caret positions agree.
...
export function createTextInputView(
	input: Node.Type,
	fontSize: number,
	singleLine = false,
	background = 0xfffafbf5,
	borderless = false,
	fontName = goTheme.font,
) {
	// Draw in scene order so modal surfaces cannot cover the field chrome.
	const fill = DrawNode();
	fill.drawPolygon(roundedRectVerts(input.width, input.height, 12), Color(background));
	input.addChild(fill, -2);
	const border = DrawNode();
	border.tag = `${input.tag}-border`;
	border.color3 = Color3(0xc8d1bb);
	border.drawPolygon(roundedRectVerts(input.width, input.height, 12), Color(0), 1, Color(0xffffffff));
	input.addChild(border, -1);
	fill.visible = !borderless;
	border.visible = !borderless;
	const insetX = 12,
		insetY = 8;
	...
	const makeLabel = () => {
		const label = Label(fontName, fontSize, true);
	...
	placeholder.textWidth = singleLine ? -1 : width;
	placeholder.y = textTop;
	placeholder.color3 = Color3(0x8b927e);
	clip.addChild(placeholder);
	const caret = DrawNode();
	...
	const position = () => {
		...
		content.y = singleLine ? 0 : offset;
		label.y = textTop;
		const stop = layout.stops[index];
		caret.position = Vec2(
			singleLine ? stop.x : math.min(width, stop.x),
			singleLine ? (height - fontSize - 2) / 2 : height - stop.row * lineHeight - fontSize - 2,
		);
	};
	const follow = () => {
		const stop = layout.stops[index];
	...
	caret.schedule((dt) => {
		blink += dt;
		caret.visible = active && (blink % 1 < 0.5 || App.reducedMotion);
		return false;
	});
	...
	return {
		...
		update: (text: string, hint: string, focused: boolean, cursor: number) => {
			...
			if (lastText !== text) {
				layout = layoutInput(text, singleLine ? math.huge : width, advance);
				label.text = layout.text;
				lastText = text;
			}
			placeholder.text = hint;
			placeholder.visible = text === "" && !focused;
			border.color3 = Color3(focused ? 0xb69427 : 0xc8d1bb);
			active = focused;
			index = math.max(0, math.min(layout.stops.length - 1, cursor));
			blink = 0;
			caret.visible = active;
			follow();
		},
		caretPosition: () => Vec2(insetX + caret.x + content.x, insetY + caret.y + content.y),
		indexAt: (point: Vec2.Type) =>
			nearest(point.x - insetX - content.x, singleLine ? 0 : math.max(0, math.floor((height + offset - (point.y - insetY)) / lineHeight))),
		verticalIndex: (cursor: number, direction: number) => {
			const stop = layout.stops[math.min(cursor, layout.stops.length - 1)];
...
export function createTextInput(options: TextInputOptions) {
	...
	const normalize = (text: string) =>
		options.singleLine ? string.gsub(text, "[\r\n]", "")[0] : string.gsub(string.gsub(text, "\r\n", "\n")[0], "\r", "\n")[0];
	const updateIMEPos = (next?: (this: void) => void) => {
		...
		target.convertToWindowSpace(
			Vec2(math.max(12, math.min(target.width - 12, caret.x)), math.max(8, math.min(target.height - 8, caret.y))),
			(pos) => {
	...
	const setValue = (text: string, at: number) => {
	...
	const keyInput = (key: KeyName) => {
		...
		if (key === KeyName.BackSpace && cursor > 0) setValue(inputSlice(value, 0, cursor - 1) + inputSlice(value, cursor), cursor - 1);
		else if (key === KeyName.Delete && cursor < inputLength(value))
			setValue(inputSlice(value, 0, cursor) + inputSlice(value, cursor + 1), cursor);
		else if (
			key === KeyName.Home ||
			key === KeyName.End ||
			key === KeyName.Left ||
			...
			cursor =
				key === KeyName.Home
					...
					: key === KeyName.End
						? inputLength(value)
						: key === KeyName.Up || key === KeyName.Down
							? (view?.verticalIndex(cursor, key === KeyName.Up ? -1 : 1) ?? cursor)
							: math.max(0, math.min(inputLength(value), cursor + (key === KeyName.Left ? -1 : 1)));
			refresh();
		} else if (key === KeyName.Return) {
			const modified =
				Keyboard.isKeyPressed(KeyName.LCtrl) ||
				Keyboard.isKeyPressed(KeyName.RCtrl) ||
				Keyboard.isKeyPressed(KeyName.LGui) ||
				Keyboard.isKeyPressed(KeyName.RGui);
	...
	return {
		...
		deferFocus: () => {
			...
			thread(() => {
				sleep(0);
				if (captured === revision) focus();
			});
		},
		mount: (target: Node.Type) => {
			unmount();
			node = target;
			view = createTextInputView(target, options.fontSize, options.singleLine, options.background, options.borderless, options.fontName);
			target.touchEnabled = true;
			target.swallowTouches = true;
			target.slot("GamepadActivate", () => {
				if (options.isEnabled()) focus();
			});
			target.onAttachIME(() => {
				focused = true;
				composition = "";
			...
			target.onTextEditing((text, start) => {
				...
				compositionCursor = math.max(0, math.min(inputLength(composition), start ?? inputLength(composition)));
				refresh();
			});
			...
			target.onTapMoved((touch) => {
				...
				view?.scroll(options.singleLine ? -touch.delta.x : touch.delta.y);
				if (focused) updateIMEPos();
			});
